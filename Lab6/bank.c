...
int main()
{
	...
	if (pid == CHILD){
		...
		{   
			P(sem); 
			// Critical Section Begin, function P is called then lock
			...
			V(sem);
			// Critical Section Ended, signal to unlock with funciton V
		}
	}
		...
		if (pid == CHILD)
		{
			...
			{	
				...
				else
				{
					...
					// Created Condtional Statement for balance checking for SON_1
			...
			if (pid == CHILD)
			{
				...
				while(flag1 == FALSE) 
				{
					...
					else
					{
						...
						// Created Condtional Statement for balance checking for SON_2
						if(bal2==0 || WithdrawAmount > bal2){
                        fclose(fp2);
                        fclose(fp3);
                    	}
